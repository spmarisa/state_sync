...
No token is required. You can omit `config.github_token` entirely.
...
```ruby
config.github_token = Rails.application.credentials.dig(:github, :state_sync_token)
```
...
If you enable `auto_refresh` with `refresh_interval: 60` (every minute) and have 10 files loaded,
